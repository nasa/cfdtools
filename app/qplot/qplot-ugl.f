...
C                   Originally: DEC/OpenVMS; SGI/IRIX; CA-DISSPLA V11.5-9312
C                   This version:  Linux or Windows; UGL emulation of CA-DISSPLA
C
C
C     Notes:
C
...
C     Author:
C
C        (Originator) Robert Kennelly   (Further Development) David Saunders
C        Mail Stop 227-6                Mail Stop 230-1
...
C        October, 2008   DAS     Adaptation to UGL emulation of CA-DISSPLA needs
C                                changes in subroutine PLOTDEV (only).
C
C-------------------------------------------------------------------------------
...
C     Local constants.
     ...
     >   MAXLIST= 9,              ! Max. number of screen OR metafile options
...
C
C        This version has been adapted to the UGL emulation package.
C
C
C USAGE:
C
...
C     --------
      ...
      DATA
     >   ALLDEV /                       ! Order should match config file
     1   'Screen preview                ',
     2   'PostScript (Black & White)    ',
     3   'PostScript (Color)            ',
     4   'HP Laser Jet II/IV            ',
     5   'PostScript (Color 2-up)       ',
     6   'UGL metafile                  ',
     7   'GIF metafile                  '/
...
C        preview (screen) devices that they are itemized here rather than
...
C        Strip trailing blanks from description of default device (LIST(1)):
         ...
         IF (TYPELIST (ITEM) (1:2) .EQ. 'PS') THEN
            PWIDTH = 7.
            CALL READR (LUNCRT,
...
CCCC     IF (IDEV .NE. 7 .AND. IDEV .NE. 8) THEN  ! Which, if any?
CCCC        CALL RESET ('HWCHAR')
            CALL IOMGR (0, -102)
CCCC     END IF

C        XPAGE and YPAGE are outputs from DEVICE, in inches:

         IF (TYPELIST (IDEV) (1:2) .NE. 'PS') THEN

            CALL DEVICE (IDEV, XPAGE, YPAGE)

         ELSE                                     ! PostScript

C           It's not clear how much of this is relevant for UGL emulation:
...
CCC         CALL PSCRPT (XPAGE, YPAGE, PWIDTH) ! CA-DISSPLA

            CALL DEVICE (IDEV,  XPAGE, YPAGE)  ! UGL substitute
...
CCC         CALL HWCHAR (20., 20., 90., 1, 'HEBREW')
