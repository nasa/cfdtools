!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   subroutine read_fieldview (lundat, lunerr, nvars, nnodes, x, y, z, flowvar, &
                              varname, nelem, element, ios)
...
!  along with density and its x/y/z partial derivatives, although the subroutine
!  will read any number of flow variables.
!
!  Up to 4 cell types are handled: tetrahedral, hexahedral, prism, and pyramid,
!  and they are expected to follow the node numbering convention that appears in
!  the FieldView Reference Manual.
!
...
!
!  The flow variables for a given point are assumed to be contiguous in the file
!  but they are returned in PLOT3D-type order for possible rotation of the
!  gradient vectors of variable 1 that are expected to be variables 2, 3, 4.
!
!  The file being read is expected to be open upon entry, and it is closed here.
!  All storage is allocated here.
!
!  History:
!
!  07/01/10  J.R.Carlson, LaRC  Original reading prog. adapted from the writing.
...
!  Local constants:
   ...
   integer, parameter :: maxelem = 4           ! Tet, hex, prism and/or pyramid
...
!  integer, parameter :: fv_nodes      = 1001  ! These aren't really used either
...
!  integer, parameter :: fv_tet_elem_id   = 1  ! Evidently unused iheader values
...
!  Boundary table:
   ...
   do ibound = 1, number_of_boundaries
      ...
      write (lunerr,'(2i5, 4x, a)') ibound_data_flag, irighthand, trim (bc_name)
   end do
...
!  Assume a read error means no more element types, and that the "fv_variables"
!  code has been encountered instead.
...
!! read  (lundat, iostat=ios) fv_variables  ! This was trapped as an error above
