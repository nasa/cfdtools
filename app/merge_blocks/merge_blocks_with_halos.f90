!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      program MERGE_BLOCKS_WITH_HALOS

!  Description:
!
!     Utility for joining or rearranging multiblock grid blocks with halo cells.
!     This adaptation of program MERGE_BLOCKS assumes the grid (and optional
!     function file) contain cell-centered data with "halo" cell points on the
!     true block boundaries.  Where two blocks join, BOTH sets of halo cells
!     need to be suppressed - not just one set.
!
...
!
!  Sample control file (merge.inp, identical to what would be used for the
!                       vertex-centered form of the grid with MERGE_BLOCKS):
!
...
!     10/25/06    "      "      MERGE_BLOCKS_WTH_HALOS adapted to deal with the
!                               unexpected complication associated with halos.
!
...
!     ----------
      ...
      write (luncrt, '(a)', advance='NO') ' Input grid with halo cells (unf): '
      ...
      write (luncrt, '(a)', advance='NO') &
         ' Cell-centered function file | none): '
      read  (lunkbd, '(a)') filename
...
!       Determine output block dimensions and check that other sums match:
        ...
        idimout(n) = 2  ! Gives original dimension if no joins
        first = .true.
        ...
        do k = 1, njoink
          do j = 1, njoinj
            ...
            do i = 1, njoini
              ...
              if (first) then
                idimout(n) = idimout(n) + mxl - 2 ! Remove both sets of halos
              else
                isum = isum + mxl - 2
              end if
...
!     Internal procedures for MERGE_BLOCKS_WITH_HALOS:
...
!       ----------------------------------------------------------
      ...
      end program MERGE_BLOCKS_WITH_HALOS
