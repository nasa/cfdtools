!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
   program combine_blocks_turb
!
!  This version of COMBINE_BLOCKS is a work-around for local damage/repair flow
!  calculations involving turbulent solutions.  Actually, by dealing instead
!  with a variable number of species, it does not matter whether the solutions
!  are laminar or turbulent.  [In the case of laminar solutions in air, disting-
!  uishing one- and two-temperature cases was feasible for cavity cases without
!  adding a prompt, but here a prompt is unavoidable.  (Jay Hyatt first added a
!  prompt for the number of turbulent flow variables in the function file, but
!  prompting for the number of species avoids the 5-species assumption.  Higher-
!  level automated procedures are affected by the number of prompts.)  Note that
!  only cavity cases (or protrusions that can mostly be treated as for cavities)
!  require the extra prompt in order to initialize flow within the cavity.]
!
...
!  cases.
!
!  Description from the earlier COMBINE_BLOCKS:
!
...
!     09/09/09  Jay Hyatt/   Jay added the prompt for # turbulent variables;
!               D. Saunders  prompting for # species makes sense too, now that
!                            a separate version appears unavoidable.
...
!  Variables:
   ...
   integer :: &
      i, ib, ios, j, nblocks_1, nblocks_2, nblocks_out, npts, nspecies,        &
      nvars_1, nvars_2
...
!  Execution:
   ...
   if (cavity_case) then
      write (luncrt, '(a)', advance='no') ' Number of species: '
...
!     Store the surface data for grid 1 (k = 1):
      ...
      nvars_2 = nvars_1
      write (luncrt, '(a, i3)') ' Number of flow variables =', nvars_2
...
!  ---------------------------------------
   ...
   do ib = 1, nblocks_2
      ...
      if (cavity_case) then ! Generate flow starting guesses
         ...
         call q_block_estimate (nblocks_1, nvars_1, nspecies, surf_1,          &
                                grid_out(i), ib, ios)
         if (ios /= 0) go to 99
...
! *** stop ! Avoid system dependencies.
   ...
   end program combine_blocks_turb

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

   subroutine q_block_estimate (nblocks_1, nvars, nspecies, surf,              &
                                block_2, iblock, ios)
...
!
!  The known number of species are assumed to appear first among the flow
!  variables, followed by three velocity components.
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!  Arguments:
   ...
   integer, intent (in)             :: nspecies         ! # species densities
...
!  Local constants:
   ...
   real,    parameter :: default = 0.1    ! For species densities -> pressure
...
!  (The original test for nspecies < 1 is redundant now that nspecies is input.)
