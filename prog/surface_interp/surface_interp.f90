...
!  Description:
!
!  Read a structured surface dataset (Tecplot ASCII) and a second target dataset
!  of one or more patches (possibly unstructured points in a simpler format).
!  Interpolate the first dataset at each target point and save results as a
!  Tecplot file and (for the simpler input format) as a tabulation for possible
!  pasting into a spreadsheet.  The output file(s) may contain just a subset of
!  the input functions.
!
...
!  Constants:
   ...
   integer, parameter :: &
      luntarget  =  1,   &  ! Input target pts. | patches (list | Tecplot ASCII)
      lunsurface =  2,   &  ! Input structured surface solution  (Tecplot ASCII)
...
!  Variables:
   ...
   integer :: &
      ib, idebug, ios, jdebug, len_buffer, m, nb_surface, nb_target, ndebug,   &
      nf_out, nf_surface, npts, nquad, target_type
   ...
   character :: &
      buffer * 80, filename_tab * 80, output_packing * 5, tab_format * 43
...
!  Special tabulation if target is really a list of body points (unstructured):
   ...
   do ib = 1, nb_target
      if (target(ib)%ni * target(ib)%nj > 1) unstructured_targets = false
   end do
...
!  Save results:
   ...
   if (unstructured_targets) then  ! Simple tabulation; %xmin = min. distance
      ...
      tab_format = '(1p, 3e13.5, 2x, 1p, nne14.6, 2x, a, e14.6)'
      write (tab_format(22:23), '(i2)') nf_out

      write (luntab, tab_format) &
         (results(ib)%x(1,1,1), results(ib)%y(1,1,1), results(ib)%z(1,1,1),    &
          results(ib)%q(:,1,1,1), trim (results(ib)%zone_title),               &
          results(ib)%xmin, ib = 1, nb_target)
...
!  Repack results as a single zone?
   ...
   if (unstructured_targets) then
      ...
      do ib = 1, nb_target
         ...
         one_zone(1)%z(ib,1,1)   = results(ib)%z(1,1,1)
...
!  Local procedures for program SURFACE_INTERP
!  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!              aft-body patches of a full CFD surface grid:
               ...
               if (target_xyz(1) > x_upper_bound) then
                  ...
                  results(ib)%q(:,i,j,1) = f_beyond_xub
                  cycle
...
!              Interpolate the surface flow at this target point:
               ...
               interp_f(:) = &
                  qm1 * (pm1 * surface(n)%q(:,ic,  jc,  1)  + &
                           p * surface(n)%q(:,ic+1,jc,  1)) + &
                    q * (pm1 * surface(n)%q(:,ic,  jc+1,1)  + &
                           p * surface(n)%q(:,ic+1,jc+1,1))
...
!              Debug output:
               ...
               if (ib == ndebug) then
                  if (i == idebug .and. j == jdebug) then
                     ...
                     write (6, '(i3, 4f20.8)') &
                       (m, surface(n)%q(m,ic:ic+1,jc:jc+1,1), m = 1, nf_surface)
                     write (6, '(a)') ' Interpolated values:'
                     write (6, '(i3,f20.8)') (m, interp_f(m), m = 1, nf_surface)
                  end if
               end if
...
!              Store the indicated subset of functions:
               ...
               do m = 1, nf_out
                  results(ib)%q(m,i,j,1) = interp_f(list_of_functions(m))
               end do
...
!     Count and read the surface points listed one per line with a label.
!     --------------------------------------------------------------------------

!     Local constants:

      character, parameter :: delimiters * 2 = ' ,'
...
!        Parse the line in order to handle descriptors with embedded blanks:
         ...
         do i = 1, 3
            ...
            if (ios /= 0) then
               write (luncrt, '(/, a, i5)') &
                  ' Trouble reading target (x,y,z).  Line #:', ib
               go to 99
            end if
...
!           In case the points aren't labeled, enter x as the zone title:
         ...
         target(ib)%x = xyz(1);  target(ib)%y = xyz(2);  target(ib)%z = xyz(3)
