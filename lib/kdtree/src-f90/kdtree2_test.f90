...
program kd_tree_test
  ...
  real(kdkind) :: rv 
  integer, parameter  :: nnn = 5  ! # choices of nn = # nearest neighbors
  integer, parameter  :: nr2 = 5  ! # choices of neighborhood radius**2
  real r2array(nr2)
  ...
  write (*, '(a)', advance='no') 'Enter data dimension d and # data points N: '
  read  (*, *) d, n
  write (*, '(a)', advance='no') &
     'Enter # neighbors to use in brute force comparisons: '
  read  (*, *) nnbrute
  ...
  call cpu_time(t1)
  write (*, '(/, f12.2, a)') real(n)/real(t1-t0), ' pts./second built as a non-rearranged tree.'

  call cpu_time(t0)
  tree2 => kdtree2_create(my_array,sort=.false.,rearrange=.true.)  ! this is how you create a tree. 
  call cpu_time(t1)
  write (*, '(f12.2, a)') real(n)/real(t1-t0), ' pts./second built as a 1st rearranged tree.'

  call cpu_time(t0)
  tree3 => kdtree2_create(my_array,sort=.true.,rearrange=.true.)  ! this is how you create a tree. 
  call cpu_time(t1)
  write (*, '(f12.2, a)') real(n)/real(t1-t0), ' pts./second built as a 2nd rearranged tree.'

  allocate(results(nnbrute), resultsb(nnbrute))


  write (*, '(/, a, i3, a, /, a)') &
     'Comparing search of', nnbrute, ' neighbors to brute force on 50 random targets.', &
     'No diagnostics mean no discrepancies found.'

  do k=1,50  ! 50 different target points searched for in brute force comparisons
     !
     !
     call random_number(rv)
...
20 format(A,': NN =',I4,':',F10.0,' searches/s in ',A)

  if (.true.) then
     ! simple testing to mimic kdtree_test_old
     !
     write (*, '(/, a, /)') 'Performance on enough target points to take at least 1 second:'
     do k=1,nnn
        sps =searches_per_second(tree2,1,nnarray(k),1.0_kdkind)
        write (*,20) 'Random  target pts.',nnarray(k),sps,'rearranged tree.'

     end do
     write (*, *)
     do k=1,nnn
        sps = searches_per_second(tree2,2,nnarray(k),1.0_kdkind)
        write (*,20) 'In-data target pts.',nnarray(k),sps,'rearranged tree.'
     end do
  else
     do k=1,nr2
     ...
     do k=1,nnn
        ...
        sps = searches_per_second(tree2,2,nnarray(k),1.0_kdkind)
        write (*,20) 'in-data pts',nnarray(k),sps,'rearr. tree.'
