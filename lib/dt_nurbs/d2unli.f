      SUBROUTINE D2UNLI (CMEM, IMEM, DMEM, IDE, IER)

C   Unlock the integer space belonging to the data entity pointed to
C     by IDE.
C
...
C
C   Long Name Alias:
C     ENTRY D2_UNLOCK_IMEM (CMEM, IMEM, DMEM, IDE, IER)
...
C     Is entity already unlocked?
      ...
      IF (.NOT. D2TSLI (CMEM, IMEM, DMEM, IDE)) THEN
         IER = +1
         GOTO 9990
      ENDIF

C     Unlock integer data space
...
C     at least partial garbage collection?
      ...
      IF ((LOWI .LT. IMEM(5)) .AND. (NHDR .NE. 0)) THEN
...
C       The DO loop bound is chosen so that it CANNOT terminate here.
        STOP 'Dynamic memory fatally corrupted - D2UNLI'
