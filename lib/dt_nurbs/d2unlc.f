      SUBROUTINE D2UNLC (CMEM, IMEM, DMEM, IDE, IER)

C   Unlock the character space belonging to the data entity pointed to
C     by IDE.
C
...
C
C   Long Name Alias:
C     ENTRY D2_UNLOCK_CMEM (CMEM, IMEM, DMEM, IDE, IER)
...
C     Is entity already unlocked?
      ...
      IF (.NOT. D2TSLC (CMEM, IMEM, DMEM, IDE)) THEN
         IER = +1
         GOTO 9990
      ENDIF

C     Unlock character data space
...
C     at least partial garbage collection?
      ...
      IF ((LOWC .LT. IMEM(4)) .AND. (NHDR .NE. 0)) THEN
...
C       The DO loop bound is chosen so that it CANNOT terminate here.
        STOP 'Dynamic memory fatally corrupted - D2UNLC'
