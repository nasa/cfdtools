      SUBROUTINE D2PJIC (CMEM, IMEM, DMEM, IEG, T, IPJ, IER)

C     PURPOSE:
C        Point on Joined Initialize on Curve.  Initialize the Point
...
C     Long name alias:
C        ENTRY D2_INITIALIZE_POINT_CURVE (CMEM, IMEM, DMEM, IEG,
C    +         T, IPJ, IER)
      ...
      INTEGER           IEG, IPJ, ITS, IER
      DOUBLE PRECISION  T

      INTEGER           IDMY, NWORK, IERX
...
C     Check the validity of the IPJ Point entity
...
C     Zero the ITS pointer, since we may be initializing to a
C     completely different surface.

      IF (ITS .NE. 0) THEN
         CALL D2STEI (CMEM, IMEM, DMEM, 0, 2, IPJ, IERX)
         IF (IERX .NE. 0) THEN
            IER = -999
            GOTO 9000
         ENDIF
