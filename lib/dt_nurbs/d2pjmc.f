      SUBROUTINE D2PJMC (CMEM, IMEM, DMEM, T, IPJ, IER)

C     PURPOSE:
C        Point on Joined Move on Curve.  Change the Point
C        on Joined IPJ to be the point determined by parameter value
C        T on the same Edge on which it is currently located.  Error
C        if not on an Edge.
C
...
C     Long name alias:
C        ENTRY D2_MOVE_POINT_CURVE (CMEM, IMEM, DMEM, T, IPJ, IER)
...
C     Check the validity of the Point entity and get Edge pointer
C     IEG

      CALL D0PJFP (CMEM, IMEM, DMEM, IPJ, LABELX, IEG, IDMY, 
