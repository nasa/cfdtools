      SUBROUTINE D1IGFD (CMEM, IMEM, DMEM, IHDR, IGLBL, IPAR,
     +       DVAL, IER)

C     Fetch a double precision element from an IGES entity or an IGES
C     Index entity.
C
...
C   OUTPUT:
C     DVAL    Double precision element to fetch.
C     IER     Returned error value.  Zero implies no errors.  Otherwise,
C             -9  = TYPCOD for this element is not 'E', 'D' or 'O'
...
C     D1FEED   Store an double precision element
C
C   HISTORY:
...
C*****
      ...
      INTEGER        IMEMX, ITCDX, DMEMX, IERX
      CHARACTER*(1)  TYPCOD
      LOGICAL        IGLBL
...
C        Nothing to fetch
      ...
      IF ((TYPCOD .NE. 'E') .AND. (TYPCOD .NE. 'D')) THEN
         IER = -9
         GOTO 9999
      ENDIF

C     Get the index
      CALL D1FEEI (CMEM, IMEM, DMEM, IHDR, IMEMX, DMEMX, IERX)
      IF (IERX .NE. 0) GOTO 9900

C     Fetch the value
      CALL D1FEED (CMEM, IMEM, DMEM, IHDR, DMEMX, DVAL, IERX)
      IF (IERX .NE. 0) GOTO 9900
