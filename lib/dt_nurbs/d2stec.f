      SUBROUTINE D2STEC (CMEM, IMEM, DMEM, CELM, JSUB, IDE, IER)
...
C        D1STEC   Store character element (shadow)
C        DTERR    Error handler
C
C     HISTORY:
C        12Mar92  D. Parsons   Created.
C        09Jun93  D. Parsons   Removed guts to D1STEC
C
C     ------

C     Long name alias:
C        ENTRY D2_STORE_CMEM_ELEMENT (CMEM, IMEM, DMEM, CELM, JSUB,
C    +         IDE, IER)
...
C     Call the general pointer-check utility routine
      ...
      CALL D1STEC (CMEM, IMEM, DMEM, CELM, JSUB, IHDR, IER)
