      SUBROUTINE D0G141 (CMEM, IMEM, DMEM, IGEHDR, IGI, IER)

C     Erase an IGES type 141 (Boundary) Entity and its assumed
C     "physically dependent" entities, namely the model space and
C     parameter space curve entities.  Do not erase the surface entity.
C
...
C             +1  = Could not erase all subordinate curve entities.
...
C*****
      ...
      INTEGER IHDR, ITYP, IERX, IBGE, IXI, I, J, NBDRY, NDEX, NP
...
C        We have a good Index, locate the list of MEM pointers to Enitites
         IXGI = IMEM(IHDR+2) + 1
         NDEX = IMEM(IXGI)/2
C        Prepare to loop through the boundary model space curves
         IXI = IXI + 22
         NBDRY = IMEM(IXI)
         DO 200 I=1,NBDRY
            IGDEX = (IMEM(IXI+1) + 1)/2
            IF (IGDEX .GT. 0 .AND. IGDEX .LE. NDEX .AND.
     +          2*IGDEX .NE. IMEM(IXI+1)) THEN
...
C                 The object is an IGES Entity
                  ...
                  IF (ITYP .EQ. 126) THEN
C                    The IGES Entity is a B-spline Curve (type 126),
C                    disconnect and erase it.
                     IMEM(IXGI+IGDEX) = 0
                     CALL D0GEER (CMEM, IMEM, DMEM, IHDR, JGI, IERX)
                     IF (IERX .NE. 0)  IER = 1
C                 ELSE IF (other types of known curve entities) THEN
C                    erase them
                  ELSE
                     IER = 1
                  ENDIF
               ENDIF
            ENDIF
C           Prepare to loop through the parameter space curve segments of
C           this boundary curve
            IXI = IXI + 3
            NP = IMEM(IXI)
            DO 100 J=1,NP
               IGDEX = (IMEM(IXI+J) + 1)/2
               IF (IGDEX .GT. 0 .AND. IGDEX .LE. NDEX .AND.
     +             2*IGDEX .NE. IMEM(IXI+J)) THEN
