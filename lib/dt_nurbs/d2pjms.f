      SUBROUTINE D2PJMS (CMEM, IMEM, DMEM, U, V, IPJ, IER)

C     PURPOSE:
C        Point on Joined Move on Surface.  Change the Point
C        on Joined IPJ to be the point determined by parameter values
C        (U,V) on the same Surface on which it is currently located.
C        If moving away from an Edge, the Edge pointer is nulled.
C        Error if not on a Surface.
C
C
...
C     Long name alias:
C        ENTRY D2_MOVE_POINT_SURFACE (CMEM, IMEM, DMEM, U, V, IPJ, IER)
...
C     Check the validity of the IPJ entity and get ITS
