!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
      subroutine cutsurfi_nf (IDIM, JDIM, ibeg, iend, jbeg, jend, nf,
     >                        MNODE, MEDGE, x, y, z, f, iblank, p, s,
     >                        epsxyz, nnode, nedge, ndg, xyzf, ierr)
!
!  Description:
!
!     This variant of subroutine cutsurf allows for more than one function
!     accompanying the surface coordinates, and treats the iblanked surface
!     case.  As with cutsurf_nf, the confusing order of the original
!     argument list has been revised to place inputs ahead of outputs.
!     The epsf argument has been dropped; x/y/z matching tests should suffice.
!     This version skips any triangles for which any vertex has iblank <= 0.
!
!     The remaining description is retained from cutsurf:
!
!     Cutsurf cuts a structured surface mesh with a specified plane, using
!     unstructured surface mesh techniques, by treating quadrilateral cells
!     as two triangles.  A scalar function (now 1 or more) is interpolated
!     along with the coordinates representing the cut.  More precisely,
!     cutsurf finds oriented line segments, or edges, which lie on the
!     intersection of a structured surface patch with a scalar function on
!     it (x,y,z,f) and a plane specified by a point and a surface normal
!     vector (p,s).  Edges are stored as points (xyzf) and oriented edge
!     numbers (ndg).  Cutsurf may be called repeatedly to accumulate edges
!     if nnode and nedge are left alone between calls.
!
...
!
!        |x1-x2| <= epsxyz  and  |y1-y2| <= epsxyz  and  |z1-z2| <= epsxyz
!
!     The caller must set nedge = 0 to start a new cut, and should usually set
!     nnode = 0 at the same time.  The edges may be ordered into a collection
!     of unbroken polygonal curves by using subroutine cutorder_nf.
!
!  History:
!
!     01-APR-1998  S.Thomas    Initial adaptation of unstructured techniques.
!     09-APR-1998     "        Companion cutorder eases plotting of cuts.
!     18-AUG-1998     "        Initial Fortran 90 translation.
!     20-AUG-1998  D.Saunders  Streamlined with cutorder as a library utility.
!     21-SEP-2010     "        Cutsurf_nf adapted from Cutsurf for nf >= 1.
!     13-OCT-2010     "        Cutsurfi_nf adapted from Cutsurf_nf.
!
!  Author:
!
...
!  Arguments:
      ...
      integer, intent (in) ::
     >  IDIM, JDIM,              ! Dimensions of structured surface arrays
     >  ibeg, iend, jbeg, jend,  ! Structured mesh subsurface to be processed;
     >                           ! use iend < ibeg if necessary to reverse the
     >                           ! surface normals (and/or jend < jbeg)
     ...
     >  MEDGE                    ! Max. # 2-pt. line segments handled per cut;
                                 ! MEDGE = IDIM x JDIM is an upper bound

      real, intent (in),
     >  dimension (IDIM,JDIM) ::
     >  x, y, z                  ! Structured surface mesh coordinates and ...

      real, intent (in) ::
     >  f(nf,IDIM,JDIM)          ! ... associated scalar surface function(s)

      integer, intent (in),
     >  dimension (IDIM,JDIM) :: ! Any triangle containing a vertex for which
     >  iblank                   ! iblank <= 0 is skipped

      real, intent (in) ::
     >  p(3),                    ! Point p(1:3) and vector s(1:3) from p ...
     >  s(3),                    ! ... define the cutting plane.  E.g., for a
     >                           ! cut at z = Z, use p = (0,0,Z) & s = (0,0,1.)
     >  epsxyz                   ! Tolerance for coincident pts. such as the
                                 ! data ranges * 1.E-12; see usage above
     ...
     >  xyzf(3+nf,MNODE)         ! Packed (x,y,z,f) coordinates of end points
                                 ...
                                 ! no coincident points according to epsxyz
      integer, intent (out) ::
...
!     Local variables:
      ...
      integer ::
     >   i, istart, istep, istop, j, jstart, jstep, jstop
      real ::
     >   a(3+nf), b(3+nf), c(3+nf), d(3+nf), diag1s, diag2s
...
!     Permit reorientation.  This feature can be used to reverse the
!     surface normals and hence the orientation of the edges of the
!     intersection between the structured patch and the plane.
!
      if (ibeg < iend) then
        istart = ibeg
        istop  = iend - 1
...
!     Treat a pair of triangles for each quadrilateral, using the shorter
!     of two diagonals.

      do j = jstart, jstop, jstep
        do i = istart, istop, istep

!         A quadrilateral is formed at a,b,c,d where
!
!         a = x,y,z at (i      ,j      ),
!         b = x,y,z at (i+istep,j      ),
!         c = x,y,z at (i+istep,j+jstep),
!         d = x,y,z at (i      ,j+jstep).
...
!
          ...
          b(4:) = f(:,i+istep,j)
!
          c(1)  = x(i+istep,j+jstep)
          c(2)  = y(i+istep,j+jstep)
          c(3)  = z(i+istep,j+jstep)
          c(4:) = f(:,i+istep,j+jstep)
!
          d(1)  = x(i,j+jstep)
          d(2)  = y(i,j+jstep)
          d(3)  = z(i,j+jstep)
...
!         Form two triangles by picking the smaller diagonal.

          diag1s = (a(1)-c(1))**2 + (a(2)-c(2))**2 + (a(3)-c(3))**2
          diag2s = (b(1)-d(1))**2 + (b(2)-d(2))**2 + (b(3)-d(3))**2

!         Cut two triangles for each quadrilateral cell:

          ierr = 0

          if (diag1s <= diag2s) then

            if (iblank(i,j)             > 0 .and.
     >          iblank(i+istep,j+jstep) > 0) then

              if (iblank(i+istep,j)     > 0) then
              ...
              if (iblank(i,j+jstep) > 0) call cutsurft (a, c, d)

            end if

          else

            if (iblank(i+istep,j) > 0 .and.
     >          iblank(i,j+jstep) > 0) then

              if (iblank(i,j)     > 0) then
              ...
              if (iblank(i+istep,j+jstep) > 0) call cutsurft (b, c, d)
...
!  Internal procedures for cutsurfi_nf:  cutsurft, cutsurf1, cutsurf2, cutsurf3
...
!     xyzf, ndg but do not exceed MNODE, MEDGE.  Return ierr = 0 if abc
!     does not touch the plane or if one or more line segments are found
!     in the cutting plane, else ierr /= 0 is as described in cutsurf.
!     E.g., if another line segment would make nedge > MEDGE then ierr
!     is set to 2 followed by an immediate return.
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

!     Arguments:

      real, intent (in) :: a(3+nf), b(3+nf), c(3+nf)
...
!     Arguments:
      ...
      real, intent (in) :: a(3+nf), b(3+nf), q(3), s(3)
...
!     Arguments:
      ...
      real, intent (in) :: a(3+nf), b(3+nf), c(3+nf), db, dc, q(3), s(3)

!     Local variables:

      real :: d(3+nf), det, sigma
...
!
!     Add segment ab to xyzf and ndg unless it is already there.
...
!       Add the line segment if it is not there already.
        ...
        do n = 1, nedge
          ...
          if (abs (xyzf(1,n1) - a(1)) <= epsxyz .and.
     >        abs (xyzf(2,n1) - a(2)) <= epsxyz .and.
     >        abs (xyzf(3,n1) - a(3)) <= epsxyz .and.
     >        abs (xyzf(1,n2) - b(1)) <= epsxyz .and.
     >        abs (xyzf(2,n2) - b(2)) <= epsxyz .and.
     >        abs (xyzf(3,n2) - b(3)) <= epsxyz) then
...
!
          do n = 1, nnode
            if (abs (xyzf(1,n) - a(1)) <= epsxyz .and.
     >          abs (xyzf(2,n) - a(2)) <= epsxyz .and.
     >          abs (xyzf(3,n) - a(3)) <= epsxyz) then
              ndg(1,nedge) = n
              go to 10
            end if
          end do
...
!         Add the second point if it is not there already.
          ...
          do n = 1, nnode
            if (abs (xyzf(1,n) - b(1)) <= epsxyz .and.
     >          abs (xyzf(2,n) - b(2)) <= epsxyz .and.
     >          abs (xyzf(3,n) - b(3)) <= epsxyz) then
              ndg(2,nedge) = n
              go to 20
            end if
          end do
