!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
      subroutine cutsurfi (x, y, z, f, iblank, ibeg, iend, jbeg, jend,
     >                     IDIM, JDIM, xyzfcut, nnode, MNODE,
     >                     ndgcut, nedge, MEDGE, p, s,
     >                     epsxyz, epsf, ierr)
!
!  Description:
!
!     This is a version of cutsurf for the iblanked surface case.
!     The description from cutsurf (below) still applies, but this version
!     skips any triangles for which any vertex has iblank <= 0.
!     Internal procedures and companion routine cutsurf is unaffected.
!
...
!     as two triangles.  A single scalar function is interpolated along with
!     the Cartesian coordinates representing the cut.  More precisely,
...
!     edge numbers (ndgcut).  Cutsurf may be called repeatedly to
...
!     03-MAR-2010     "        Cutsurfi adapted from cutsurf for iblanked cases.
!
!  Author:
!
...
!  Arguments:
     ...
     >  f                        ! ... associated scalar surface function
...
!     Internal procedures for cutsurfi:  cutsurft, cutsurf1, cutsurf2, cutsurf3
