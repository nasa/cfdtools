!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
      subroutine cutsurf_nf (IDIM, JDIM, ibeg, iend, jbeg, jend, nf,
     >                       MNODE, MEDGE, x, y, z, f, p, s, epsxyz,
     >                       nnode, nedge, ndg, xyzf, ierr)
!
!  Description:
!
!     This variant of subroutine cutsurf allows for more than one function
!     accompanying the surface coordinates.  The confusing order of the original
...
!     21-SEP-2010     "        Cutsurf_nf adapted from Cutsurf.
!
!  Author:
!
...
!         Cut two triangles for each quadrilateral cell:
          ...
          if (diag1s <= diag2s) then
            ...
            if (ierr == 0) call cutsurft (a, c, d)

          else

            call cutsurft (b, d, a)

            if (ierr == 0) call cutsurft (b, c, d)
...
!  Internal procedures for cutsurf_nf:  cutsurft, cutsurf1, cutsurf2, cutsurf3
