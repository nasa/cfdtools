!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
   subroutine graddis3d2 (ndat, xdat, ydat, zdat, sdat, fdat, n, power, &
                          ismooth, lunout, x, y, z, s, ier)
!
!  GRADDIS3D2 is a 3-space analogue of the gradient-based point redistribution
!  utility GRADDIS2, itself analogous to CURVDIS2 for curvature-based gridding.
!  It needs a profile of some function along with the 3-space geometric curve
...
!
!  GRADDIS3D2 applies geometric normalization to the coordinates and normalizes
!  the function data before calling GRADDIS3D in a loop over the power argument
...
!  other coordinates are scaled by the same factor to preserve shape, along
!  with being shifted to the origin.  The function data are also shifted and
!  normalized to [0,1] before gradients are calculated.  Larger gradient
...
!
!  11/25/13  D.A.Saunders  2-space GRADDIS2 starting point.
!  11/26/13    "      "    GRADDIS3D2 adapted from GRADDIS2.
!  12/03/13    "      "    GRADDIS3D now returns updated arc lengths, not just
...
!  Local variables:
   ...
   real    :: xdata(ndat), ydata(ndat), zdata(ndat) ! Avoided touching inputs
   real    :: sdata(ndat), fdata(ndat)
   real    :: fpdat(ndat), fpp(ndat)                ! Use fpp for unneeded fk,
...
!  Procedures:
   ...
   external :: graddis3d ! Redistribute points according to |df/ds|
...
!  Execution:
   ...
   call getscale ('G', ndim, ndat, xdata, ydata, zdata, xscale, xshift, ier_loc)
   call usescale ('N', ndim, ndat, xdata, ydata, zdata, xscale, xshift, ier_loc)
   ...
   do  ! Until a low-enough exponent allows convergence
      ...
      call graddis3d (ndat, xdata, ydata, zdata, sdata, fpdat, n, pwr, &
                      ismooth, lunout, x, y, z, s, ier)
      if (ier == 0) exit

      write (*, '(a, i3, f5.1)') &
         'GRADDIS3D2: Trouble in GRADDIS3D.  ier, power:', ier, pwr
   ...
   call usescale ('D', ndim, n, x, y, z, xscale, xshift, ier_loc)
   call usescale ('D', 1,    n, s, s, s, xscale, sshift, ier_loc)

   end subroutine graddis3d2
