C+------------------------------------------------------------------------------
C
      SUBROUTINE GRADDIS3D (NDAT, XDAT, YDAT, ZDAT, SDAT, GDAT, N,
     >                      POWER, ISMOOTH, LUNOUT, X, Y, Z, S, IER)
C
C     Acronym:  GRADient-based DIStribution (3-space)
C               ----           ---
C     Purpose:
C
C        GRADDIS3D is the 3-space analogue of the gradient-based utility
C     GRADDIS, itself the analogue of curvature-based utility CURVDIS.
C     GRADDIS3D is expected to be used in a loop over POWER by GRADDIS3D2.
C
C        GRADDIS3D redistributes points along a curve in 3-space which is
...
C     GRADDIS3D is intended for geometric applications where the coord-
...
C     it via GRADDIS3D2 is strongly recommended, as it deals with data
...
C        The same shape function as used by CURVDIS3D is appropriate,
...
C     X-Y-Z coordinates corresponding to the new arc lengths.
C
C        The work space is used/reused in the following sequence:
C
C     (1)  WORK(1:NDAT)               xdat-ydat-zdat relative spacings
...
C        LCSFIT           Interpolates x, y, z to the redistributed arc
C                         lengths along the parameterized curve
C        SMOOTH1D_NITERS  Explicit smoothing utility with iteration control
C
C     Error Handling:
C
C        See IER description below.  It is the user's responsibility to check
C     IER on return from GRADDIS3D.  Error messages are sent to |LUNOUT|.
C
C     History:
C        08-09/88    D.A.Saunders,  Initial implementation of CURVDIS.
C                    B.A.Nishida
C        11/20/13    D.A.Saunders   Version of GRADDIS used as GRADDIS3D start.
C        11/26/13      "     "      Initial adaptation of GRADDIS as GRADDIS3D.
C        12/02/13      "     "      Not returning the updated arc lengths
C                                   was poor design: recovering them from the
C                                   new (x,y,z)s is not identical to using the
C                                   precise arc lengths that produced those
C                                   new (x,y,z)s (for function interpolations).
C
...
C     Redistribute the curve coordinates:
      ...
      CALL LCSFIT (NDAT, SDAT, ZDAT, TRUE, TIGHT, N, S, Z, WORK(NDAT+1))

  999 RETURN

      END SUBROUTINE GRADDIS3D
