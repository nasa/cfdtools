C+----------------------------------------------------------------------
C
      SUBROUTINE EXPDIS2 (MODE, XA, XB, DX, N, X, BETA, LUNOUT)
C
C  PURPOSE:
C
...
C
C  ENVIRONMENT:  VAX/VMS FORTRAN 77 single precision, with
...
C     Local constants:
     ...
     >   SUBNAME * 7
      ...
      PARAMETER
     >  (BETAMAX = 10.D+0,       ! Search interval for the value of BETA
     >   BETAMIN = 1.00000001D+0,! which produces the desired DX
     >   MAXFUN  = 30,           ! Max. # zero-finder function evaluations
     >   ONE     = 1.D+0,
     >   SUBNAME = 'EXPDIS2',
     >   THREE   = 3.D+0,
     >   TOL     = 0.D+0,        ! Ask ZERORC for full precision
     >   TWO     = 2.D+0)
...
C     Set up for DOUBLE PRECISION internal arithmetic:
...
C        Symmetric bunching towards both XA and XB:
         ...
         DO I = 1, (N + 1) / 2
            FI = EXP (RNM1 * DBLE (2*I-N-1))
            X (I) = REAL (RANGE * (ONE + DBLEBETA * (FI - ONE) /
     >                                              (FI + ONE))) + XA
...
C        Bunch towards XB:
         ...
         DO I = 1, N
            FI = EXP (RNM1 * DBLE (I - 1))
            X (I) = REAL ((RANGE * DBLEBETA) * (FI - ONE) / (FI + ONE))
     >              + XA
         END DO
...
C        Bunch towards XA:
         ...
         DO I = 1, N
            FI = EXP (RNM1 *  DBLE (N - I))
            X (I) =
     >        REAL (RANGE * (ONE - DBLEBETA * (FI - ONE) / (FI + ONE)))
     >        + XA
         END DO
...
C     Formats:
 ...
 1000 FORMAT (///, ' EXPDIS2:  Bad return from ZERORC. ISTAT = ', I2,
     >        /, ' XA, XB, DX =', 1P, 3E15.6,
     >        /, ' N = ', I4, '   MODE = ', I1,
