C+------------------------------------------------------------------------------
C
      SUBROUTINE RIPPLE2P (MDIM, NDIM, M1, M2, N1, N2, XY, XP, YP,
     >                     M, N, EPS, P, Q, STATUS)
C
C     ONE-LINER:  "RIPPLE" search of a 2D quasi-rectangular grid (Pairs form)
C                  ------              -                          -
C     PURPOSE:
C
C        RIPPLE2P is a variant of RIPPLE2D for an (X,Y) grid stored as pairs.
C
C        RIPPLE2P performs a search within a quasi-rectangular 2D grid for
...
C
C
C        Subroutine BILINTP is used to perform the test.  BILINTP returns
...
C     N1,
C     N2
C     XY (2,MDIM,NDIM) R   I         Grid coordinates stored as pairs
...
C     EPS      -       R   I         Search tolerance used by BILINTP, q.v.
C     P,       -       R   O         Bilinear interpolation coefficients;
C     Q                              see BILINTP for their usage
...
C                                    by BILINTP is not tested for here.)
C
C     PROCEDURES:
C
C        BILINTP    Performs test for enclosing cell and returns P, Q
C
C     ENVIRONMENT:
C
...
C        01/12/01      "       RIPPLE2P adapted from RIPPLE2D for (1:2,*) case.
C
C     AUTHOR:
C
...
C     Arguments.
      ...
      REAL       XY (2, MDIM, NDIM), XP, YP, EPS, P, Q
...
C     Begin with the initial guess.
      ...
      CALL BILINTP (MDIM, NDIM, XY, XP, YP, M, N, EPS, P, Q, STATUS)
...
C           Avoid repeated tests.
            ...
            IF (ITEST .OR. JTEST) THEN
               ...
               CALL BILINTP (MDIM, NDIM, XY, XP, YP, I, J, EPS,
     >                       P, Q, STATUS)
...
C              Limit the search to the interior points of the edge.
               ...
               DO I = ISTART, IEND
                  DO J = JSTART, JEND
                     ...
                     CALL BILINTP (MDIM, NDIM, XY, XP, YP, I, J,
     >                             EPS, P, Q, STATUS)
...
C     Ensure "lower left" as for INTERVAL's 1-D search:
      ...
      IF (P .GE. ONE) THEN
         IF (M + 1 .LT. M2) THEN
            M = M + 1
            CALL BILINTP (MDIM, NDIM, XY, XP, YP, M, N, EPS, P, Q,
     >                    STATUS)
         END IF
      END IF
