C+-----------------------------------------------------------------------------+
C
      subroutine HSORTII ( ilist, iperm, Length )
C
C Title: Sort integer list.
C
C Purpose:
...
C   iperm whenever ilist is changed.  Since HeapSort is a sort-in-place
...
C               less
C****         if( rd .LT. ru  .AND.
C****&            ilist(rd) .LT. ilist(rd+1) ) rd = rd +1
              if( rd .LT. ru )then
                  if( ilist(rd) .LT. ilist(rd+1) ) rd = rd +1
              endif

C             >>sift down until the right element is sorted
              if( ilist(rd) .GE. iltemp )then
                  ilist(ld) = ilist(rd)
                  iperm(ld) = iperm(rd)
                  ld = rd
                  rd = rd + rd
