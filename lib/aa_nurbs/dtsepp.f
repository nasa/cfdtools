      SUBROUTINE DTSEPP (C, NPTS, NDIMV, WORK, NWORK, V, IER)
C
...
C     USAGE
C
C         DOUBLE PRECISION C(NC), WORK(NWORK), V(NDIMV, NDEP)
C         CALL DTSEPP (C, NPTS, NDIMV, WORK, NWORK, V, IER)
C
C         WHERE
C             NC IS THE LENGTH OF THE C ARRAY.
C             NDEP IS THE NUMBER OF DEPENDENT VARIABLES.
C                 NDEP = -1-C(2)  FOR A RATIONAL SPLINE (IE., C(2) < 0)
C                 NDEP = C(2)     OTHERWISE
C
C
C     INPUT
C
...
C
C     NOTE:
C     DTSEPV seemed to be the corrected version.  It has been renamed as
C     DTSEPP to match the User's Manual, and the incorrect DTSEPP has been
C     eliminated.  (David Saunders, NASA Ames, 3/17/92)
C
      PARAMETER (MAXD=4)
      EXTERNAL DTSPVL, DTMCON
      INTEGER NPTS, NWORK, IER
      DOUBLE PRECISION C(*), WORK(*), V(NDIMV,*), T, TSTART, TEND
      DOUBLE PRECISION VTMP(MAXD)
      DOUBLE PRECISION DT, DTMCON
      INTEGER IP, MODE, NEED, NDEP, I, J, ISTAT
C
      CHARACTER*8 SUBNAM
      DATA SUBNAM /'DTSEPP  '/
C
...
C
      ...
      ENDIF
      NDEP = INT(C(2))
      IF (ABS(NDEP) .GT. MAXD) THEN
          IER = -52
          GOTO 9900
      ENDIF
      IF (NDEP .LT. 0) NDEP = -1 - NDEP
C
