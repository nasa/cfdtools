C+------------------------------------------------------------------------------
C
      SUBROUTINE FMINRC2 (AX, BX, XMIN, FMIN, TOL, NUMFUN, CALLER,
     >                    LUNOUT, ISTAT)
C
C     FMINRC2 is simply a renamed copy of FMINRC to allow nested minimization
C     loops.  This is easier than adding a "HOLD" argument array as has been
C     done with the ZERORC analogue.
C
C     The remaining documentation is as for FMINRC.
C
C     Oneliner:
C
...
C
C     11/17/10     "    "     FMINRC2 made as a copy of FMINRC to allow a pair
C                             of nested iterations when two nonlinear variables
C                             are being optimized.
C-------------------------------------------------------------------------------
...
C     Formats:
 ...
 1010 FORMAT (/, ' FMINRC2:  Iteration history for calls by ', A, //
     >        ' Iter', 23X, 'X', 21X, 'F(X)')
 1020 FORMAT (1X, I3, 1P, 2E25.15)
 1030 FORMAT (/, ' FMINRC2:  AX > BX is illegal: ', 1P, 2E24.16)
 1040 FORMAT (/, ' FMINRC2:  Suspicious input for (max.) NUMFUN: ', I10)
 1050 FORMAT (/, ' FMINRC2:  Cannot start a new minimization before',
     >       ' the previous one is done.', /, ' Offending module: ', A)

      END SUBROUTINE FMINRC2
