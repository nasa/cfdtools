!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
   subroutine detect_flatness (n, curvature, nflmax, ifl, ifr, nfl)
!
!  Purpose:
!
...
!  Local constants:
   ...
   real, parameter ::    &
      flatmax = 0.5         ! Curvature value below which we say "flat";
...
!! write (6, '(a, i4)') '   n:', n
   ...
   do  ! Until i > n
!!    write (6, '(i4, es18.6)') i, curvature(i)
      if (abs (curvature(i)) < flatmax) then  ! Determine the extent of flatness
         nfl = nfl + 1
!!       write (6, '(a, i4)') '   nfl:', nfl
         ifl(nfl) = i
         ifr(nfl) = i
         do j = i+1, n
!!          write (6, '(i7, es18.6)') j, curvature(j)
...
!! write (6,  '(i4, 2i6)') (i, ifl(i), ifr(i), i = 1, nfl)
