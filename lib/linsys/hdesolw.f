C+------------------------------------------------------------------------------
C
      SUBROUTINE HDESOLW (MDIM, M, N1, A1, W, X, SSQMIN)
C
C  Purpose:
C
C     HDESOLW is a variant of HDESOL for the weighted linear least squares case.
C     The weights are simply applied to both the left and right sides of the
C     system of equations being solved (square or overdetermined).
C     The description from the original HDESOL follows:
C
...
C     W(M)      contains the weights applied to each equation.
...
C     3/19/2010   "   "     HDESOLW adapted from HDESOL.
C
C-------------------------------------------------------------------------------
...
C  *  Arguments:
      ...
      REAL,    INTENT (INOUT) :: A1(MDIM,N1), W(M), X(M)
...
C     Apply the weights to the left and right sides:

      DO I = 1, M
         A1(I,:) = W(I) * A1(I,:)
      END DO

C     The remainder is exactly as for HDESOL:
