C+----------------------------------------------------------------------
C
      SUBROUTINE PLSINTRP (NDATA, X, Y, I1, I2, METHOD, NEVAL, XEVAL,
     >                     YEVAL, YPEVAL)
C
C     Two-liner:  Storage-efficient parametric local cubic spline
C     ----------  interpolation (X,Y-space; Y for given X)
C
C     Description and usage:
C     ----------------------
C
C        PLSINTRP applies the local method of LCSFIT to the parametric
C     case where X and Y are treated as functions of T (arc length) and
C     Y is required for a given X as opposed to X and Y for a given T.
C     It is intended for applications (such as F(X,Y) interpolation)
C     where interpolation along more than one curve at a time would
C     make the earlier PSFIT/PSEVAL approach very inefficient (lots of
C     repeated calculation of entire arc lengths for the conventional
C     spline abscissas vs. the present method's repeated calculation of
C     just 4 values of T as needed, since LCSFIT is a 4-point method).
C     Storage of spline coefficients is also avoided, of course, since
C     that is LCSFIT's main virtue - they are computed as needed.
C
...
C     the target X, then evaluating Y for that T.  Locating T is a
...
C
C        PLSINTRP cannot be made as efficient as LCSFIT for repeated
C     calls with the same data (since it uses LCSFIT for both X and Y
C     vs. T), but repeated use with NEVAL = 1 is still reasonably
C     efficient.
C
C        As with LCSFIT, a piecewise linear option is included, and an
C     interpolated first derivative is returned with the function value.
C     The second derivative is omitted because Y" is not guaranteed to
C     be continuous by local methods.
C
...
C     advisable).
C
C        See PLSINTYZ for the (X,Y,Z)-space analogue (Y,Z for given X).
C
C     Arguments:
C     ----------
C
...
C
C     YPEVAL  R (NEVAL)       O      Interpolated DY/DX value(s).
...
C     --------
C
C     06/19/91  D.A.Saunders  PLSINTRP adapted from LCSFIT for embedding
C                             in other utilities where reasonable speed
C                             and workspace efficiency are desirable.
C                             Trying to work with at most 4 points for
C                             each call to LCSFIT is more tricky than
C                             bargained on!
C                             Debug printout to unit 6 if convergence
C                             fails has been retained: it should point
C                             to programmer error in the application.
C     07/14/91    "     "     Tightened (relative) convergence tolerance
C                             a bit: TOL = 6.E-7 instead of 1.E-6.  Note
C                             that results will not be exactly equal and
C                             opposite for a symmetric airfoil in wrap-
C                             around form, since T(1:4) involves points
C                             that correspond but are in opposite order.
C     07/24/91    "     "     The test for being in the same interval
C                             as before was forgetting the descending
C                             Xs case.  And LCSFIT's 'NEW' need not be
C                             .TRUE. during the Newton iteration.
C-----------------------------------------------------------------------
...
C     Local variables.
      ...
      INTEGER
     >   IEVAL, ITER, I3, J1, J2, LEFT, N, NT
      REAL
     >   ARROW, T (4), TI, TMAX, TMIN, TOLER, XE, XI, XPI, YI, YPI
      LOGICAL
     >   ENDPT, NEW
...
C        End points can cause trouble if dX / dT ~ 0.  Avoid them:
         ...
         IF (XE .EQ. X (I1)) THEN   ! Need LCSFIT for the derivative
            ENDPT = .TRUE.
            CALL LCSFIT (3, X (I1), Y (I1), NEW, METHOD, 1, XE, YI, YPI)
            YPEVAL (IEVAL) = YPI
            YEVAL  (IEVAL) = Y (I1)
         ...
         ELSE IF (XE .EQ. X (I2)) THEN
            ENDPT = .TRUE.
            I3 = I2 - 2
            CALL LCSFIT (3, X (I3), Y (I3), NEW, METHOD, 1, XE, YI, YPI)
            YPEVAL (IEVAL) = YPI
            YEVAL  (IEVAL) = Y (I2)
...
C           LCSFIT requires only the neighboring 4 points (3 at a boundary).
            ...
            T (2) = CHORD (X, Y, J1, J1 + 1)
            T (3) = CHORD (X, Y, J1 + 1, J1 + 2) + T (2)
            T (4) = CHORD (X, Y, J2 - 1, J2) + T (3)    ! Not used at ends
...
C           Find where F (T) = X (T) - XE = 0.   Note that F'(T) = X'(T).
   ...
   30       CONTINUE
     ...
     >                  '0** PLSINTRP: Bad input data likely.',
...
C           Evaluate Y at this T:

            NEW = .TRUE.

            CALL LCSFIT (NT, T (1), Y (J1), NEW, METHOD, 1, TI, YI, YPI)

            YPEVAL (IEVAL) = YPI / XPI
            YEVAL  (IEVAL) = YI
